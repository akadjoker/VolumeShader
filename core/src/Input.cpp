...
glm::vec2 Mouse::currentPosition;
...
glm::vec2 Mouse::currentWheelMove;
glm::vec2 Mouse::previousWheelMove;
glm::vec2 Mouse::offset;
glm::vec2 Mouse::scale;
bool Mouse::cursorRelative = false;
bool Mouse::do_whell = false;
...
void Mouse::Init()
{
    ...
    scale=glm::vec2(1, 1);
    currentPosition=glm::vec2(0, 0);
    deltaPosition=glm::vec2(0, 0);
    currentWheelMove=glm::vec2(0, 0);
    previousWheelMove=glm::vec2(0, 0);
    do_whell = false;
...
{
   // LogInfo("setMousePosition(%i, %i, %i, %i)", x, y, xrel, yrel);
  
        currentPosition.x = (float)x;
        currentPosition.y = (float)y;
        deltaPosition.x=(float)xrel;
        deltaPosition.y=(float)yrel;
